...
function getSortedGpuList(gpu) {
  ...
  arr.sort((a, b) => gpuRank(gpuName(b)) - gpuRank(gpuName(a)));
  return arr;
}
...
function renderOverview(root) {
  ...
  const gpuNames = gpuList.map((g) => safe(gpuName(g))).filter(Boolean);
  ...
  const biosVer = safe(bios.SMBIOSBIOSVersion || bios.smbiosbiosversion || hh.bios_version);
  const biosBrand = safe(bios.Manufacturer || bios.manufacturer || hh.bios_vendor);
  const trBios = document.createElement("tr");
  trBios.innerHTML = `<td>BIOS</td><td>${biosVer}</td><td>品牌：${biosBrand}</td>`;
  tb.appendChild(trBios);

  const boardModel = safe(baseboard.Product || baseboard.product || hh.board_name);
  const boardBrand = safe(baseboard.Manufacturer || baseboard.manufacturer || hh.board_vendor);
...
function renderHardware(root) {
  ...
  if (gpuList.length === 0) {
    ...
    const lines = gpuList.map((g) => {
      if (typeof g === "string") return safe(g);
      return `${safe(g.Name || g.name)} / 驱动 ${safe(g.DriverVersion || g.driverversion)} / 状态 ${safe(g.Status || g.status)}`;
    });
...
function renderSoftware(root) {
  ...
  const softwares = Array.isArray(inv.installed_software)
    ? inv.installed_software
    : (Array.isArray(inv.installed_packages) ? inv.installed_packages : []);
  if (softwares.length === 0) {
    ...
    softwares.forEach((s) => {
      const tr = document.createElement("tr");
      tr.innerHTML = `<td>${safe(s.DisplayName || s.displayname || s.name)}</td><td>${safe(s.DisplayVersion || s.displayversion || s.version)}</td><td>${safe(s.Publisher || s.publisher || s.manager)}</td>`;
...
function renderAdvanced(root) {
  ...
  const hh = adv.hardware_health || {};
  const hasWindowsLike = hh.battery || hh.bios || hh.baseboard;
  if (hasWindowsLike) {
    ["battery", "bios", "baseboard"].forEach((k) => renderAdvancedValue(bHard, k, hh[k]));
  } else {
    Object.entries(hh).forEach(([k, v]) => renderAdvancedValue(bHard, k, v));
  }
  ...
  const perf = adv.performance_diagnostics || {};
  addKV(bPerf, "CPU 占用(%)", safe(perf.cpu_percent ?? perf.cpu_usage_percent));
  addKV(bPerf, "磁盘繁忙(%)", safe(perf.disk_busy_percent));
  addKV(bPerf, "可用内存(MB)", safe(perf.memory_available_mb ?? "-"));
  if (perf.memory_usage_percent !== undefined) {
    addKV(bPerf, "内存占用(%)", safe(perf.memory_usage_percent));
  }
  const chips1 = document.createElement("div");
  chips1.className = "chip-row";
  const topCPU = Array.isArray(perf.top_cpu_process_names)
    ? perf.top_cpu_process_names
    : (Array.isArray(perf.top_cpu_processes) ? perf.top_cpu_processes.map((x) => x.command || x.pid || x.user).filter(Boolean) : []);
  topCPU.forEach((n) => {
  ...
  const topMem = Array.isArray(perf.top_memory_process_names)
    ? perf.top_memory_process_names
    : (Array.isArray(perf.top_memory_processes) ? perf.top_memory_processes.map((x) => x.command || x.pid || x.user).filter(Boolean) : []);
  topMem.forEach((n) => {
    const c = document.createElement("span");
    c.className = "chip";
    c.textContent = String(n);
  ...
  renderAdvancedValue(bDrv, "driver_diagnostics", adv.driver_diagnostics || {});
